...
import { Plus, X, Pencil, LogIn, Check, Search, Lock, Copy, Download } from 'lucide-react';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    localStorage.setItem('icon_app_center_version', APP_VERSION);
  }, [apps]);
  ...
  const exportConfig = () => {
    const code = `export const INITIAL_APPS: AppLink[] = ${JSON.stringify(apps, null, 2)};`;
  ...
  const iconList = useMemo(() => {
    ...
    if (!iconSearch) return allIcons.slice(0, 48); 
    return allIcons.filter(name => name.toLowerCase().includes(iconSearch.toLowerCase())).slice(0, 60);
  }, [iconSearch]);
      ...
      >
        <form onSubmit={handleSaveApp} className="space-y-4">
              ...
              {iconList.map((iconName) => {
                ...
                return (
                  <button
                    ...
                    className={`p-2 rounded-lg flex items-center justify-center transition-all ${isSelected ? 'bg-blue-100 text-blue-600 ring-2 ring-blue-500' : 'bg-white border border-gray-200 text-gray-500 hover:bg-gray-100'}`}
                  >
          ...
          <div className="pt-4 flex gap-3">
            <button type="button" onClick={() => setIsAppModalOpen(false)} className="flex-1 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-lg hover:bg-gray-50 font-medium transition-colors">Cancel</button>
            <button type="submit" className="flex-1 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 font-medium shadow-sm transition-colors">{currentApp.id ? "Save Changes" : "Create App"}</button>
          </div>
        </form>
      </Modal>
    </div>
